...
import { CalendarDay, CalendarMonth, CalendarModalOptions } from '../calendar.models';
import { CalendarService } from '../calendar.service';
import { pickModes } from '../utils/config';
...
export class CalendarModalComponent implements OnInit, AfterViewInit {
  ...
  datesTemp: Array<CalendarDay> = [{} as CalendarDay, {} as CalendarDay];
  ...
  init(): void {
    ...
    this.calendarMonths = this.calSvc.createMonthsByPeriod(
      ...
      this.findInitMonthNumber(this._d.defaultScrollTo ?? new Date()) + this.step,
      this._d
    );
  }
  ...
  initDefaultDate(): void {
    ...
    switch (pickMode) {
      ...
      case pickModes.MULTI:
        ...
        break;
      default:
        this.datesTemp = [{} as CalendarDay, {} as CalendarDay];
    }
  }
  ...
  onCancel(): void {
    ...
    let value = Array<CalendarDay>();
    value = this.datesTemp ?? new Array<CalendarDay>();
    this.modalCtrl.dismiss(this.calSvc.wrapResult(value, this._d.pickMode ?? ''), 'done');
  }
  ...
  scrollToDefaultDate(): void {
    this.scrollToDate(this._d.defaultScrollTo ?? new Date());
  }
