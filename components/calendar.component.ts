...
export class CalendarComponent implements ControlValueAccessor, OnInit {
  ...
  switchView(): void {
    // this._view = this._view === 'days' ? 'month' : this._view === 'month' ? 'year' : 'days';
    this._view = this._view === 'days' ? 'month' : 'days';
  }
  switchIcon(): string {
    //return this._view === 'days' ? this._compatibleIcons.caretDown : this._view === 'month' ? this._compatibleIcons.caretUp : this._compatibleIcons.chevronDown;
    return this._view === 'days' ? this._compatibleIcons.caretDown : this._compatibleIcons.caretUp;
  }
  ...
  canNext(): boolean {
    if (!this.cmOptions.to || this._view !== 'days') { return true; }
    return this.monthOpt.original.time < moment(this.cmOptions.to).valueOf();
  }
  ...
  canBack(): boolean {
    if (!this.cmOptions.from || this._view !== 'days') { return true; }
    return this.monthOpt.original.time > moment(this.cmOptions.from).valueOf();
  }
  ...
  onChanged($event: CalendarDay[]): void {
    switch (this.cmOptions.pickMode) {
  ...
  _monthFormat(date: number): string {
    return moment(date).format(this.cmOptions.monthFormat?.replace(/y/g, 'Y'));
  }
  ...
  private initOpt(): void {
    ...
    this.cmOptions = this.calSvc.safeOpt(this._options || {});
  }

  createMonth(date: number): CalendarMonth {
    return this.calSvc.createMonthsByPeriod(date, 1, this.cmOptions)[0];
  }

  _createCalendarDay(value: CalendarComponentPayloadTypes): CalendarDay {
    return this.calSvc.createCalendarDay(this._payloadToTimeNumber(value), this.cmOptions);
  }
